...
function drawTimeSeriesChart() {
    ...
    // Initialize all 8 quarters (Q1-Q4 2023 and 2024) with zero values
    const allQuarters = [
    ...
    data.positiveDrugs.forEach(record => {
        ...
        const month = date.getMonth(); // 0-11 (Jan=0, Feb=1, and so on)
        let quarter = "";
    ...
    const width = Math.min(container.parentElement.clientWidth - 40, 900);
    const height = 450;
    ...
    // Create line data with x and y coordinates 
...
function drawAgeGroupChart() {
    ...
    values.forEach((value, i) => {
        ...
        // value labels for bar chart (age group view)
        if (value > 0) {
            svg.append("text")
